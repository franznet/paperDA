...
\author{Franz~Yupanqui~Machaca\,\orcidlink{0000-0003-1149-1370}, \and Pablo~Salazar~Reinoso\,\orcidlink{0000-0002-7496-705X}, \and Claudio~Meneses~Villegas\,\orcidlink{0000-0003-1112-4925}
        % <-this % stops a space
\thanks{Franz~Yupanqui~Machaca is with the PhD Program in Sustainable Engineering, Universidad Católica del Norte, Antofagasta 1270709, Chile, with the Millennium Institute on Volcanic Risk Research – Ckelar Volcanoes, Antofagasta 1270709, Chile, and also with the 4D Perceptio, La Paz, Bolivia (e-mail: frayup@gmail.com, franz.yupanqui@alumnos.ucn.cl).}
\thanks{Pablo~Salazar~Reinoso is with the Department of Geological Sciences, Universidad Católica del Norte, Antofagasta 1270709, Chile, and also with the Millennium Institute on Volcanic Risk Research – Ckelar Volcanoes, Antofagasta 1270709, Chile  (e-mail: pasalaz@ucn.cl).}
\thanks{Claudio~Meneses~Villegas is with the Department of Computing and Systems Engineering, Universidad Católica del Norte, Antofagasta 1270709, Chile  (e-mail: cmeneses@ucn.cl).}
}
...
This work pursues to characterize several data augmentation techniques in terms of similarity and diversity metrics, and evaluating their impact on the performance of classification of seismic-volcanic signatures. 
...
A primary objective of using data augmentation is to balance the dataset, ensuring greater diversity in the event feature space, and keeping a minimal similarity with the original data.
...
\caption{a) Location map of Lascar volcano. b) Red dots indicate the positions of the seismic stations.}
...
The Lascar database contains 6,145 seismic events classified by geophysicist experts belonging to CKELAR-VOLCANES. The data correspond to the period between March and October 2018. The tables \ref{table:lascar_database} and \ref{table:lascar_statistics} show the data distribution.
\begin{table*}
...
where $p_0$ is the empirical probability of agreement and $p_e$ is the expected agreement when both annotators assign labels randomly\footnote{\href{https://lightning.ai/docs/torchmetrics/stable/classification/cohen_kappa.html}{\color{blue}https://lightning.ai/docs/torchmetrics/stable/classification/cohen\_kappa.html}}.
\end{itemize}
...
By generating a simple score that accounts for model training accuracy (higher value is better), model test accuracy on real data (higher value is better), and the MS-SSIM metric value (lower value is better, indicating greater diversity), we define the Performance Score ($PS$) as follows:
\begin{equation}
...
Figure \ref{fig:da_training_msssim_interno} illustrates the concentration of points representing the MS-SSIM similarity metric against the accuracy of model training. Notably, two distant points are evident: the dataset comprising only real data (i), which has low accuracy due to being unbalanced, and the drifting dataset (vii) with a range of 0.01-0.1, which shows higher similarity. Expanding on this, figure \ref{fig:da_training_msssim_interno_zoom}, it is clear that almost all datasets demonstrate similar levels of diversity, including the real dataset; however, the augmented Jittering dataset (v) shows greater diversity.
...
\begin{itemize}
\item The first group comprises datasets that combine real and augmented data, exhibiting MS-SSIM values in the range of 0.5 to 0.55. Two cases fall outside of this range: the Drifting dataset (vi), which shows the greatest similarity with an MS-SSIM value of 0.552, and the Jittering dataset (iv), which shows the greatest diversity for this group, with a value of 0.49.
\item The second group corresponds to the fully augmented datasets, which demonstrate greater diversity than the first group. The MS-SSIM value for this group ranges from 0.3 to 0.4. Two cases fall outside this range: the Drifting dataset (vii), with an MS-SSIM value of 0.401, representing the least diversity within the group, and the Jittering dataset (v), with a value of 0.28, which shows the greatest diversity for this group and of all.
\end{itemize}
...
Regarding model performance, the Performance Score (PS), which incorporates the model training accuracy, the model testing accuracy on real data, and the MS-SSIM metric, should be examined. Figure \ref{fig:relation_index} shows that models trained on the Drifting (xi), Jittering (v), SpecAugment (xxi), and Drifting (ix) datasets exhibit the highest PS values, making them the most relevant in terms of the trade-off between model accuracy and dataset diversity. In contrast, models with low PS values, such as the Drifting dataset (vii) and the real event dataset (i), are negatively impacted by their lower diversity and imbalanced nature, respectively. This analysis conclusively demonstrates the extent to which data augmentation (DA) techniques and the inherent diversity of the dataset contribute to model performance.
...
In this work, several data augmentation techniques are applied to the classification of seismic-volcanic signals. Initially, these techniques were employed to balance the datasets and subsequently to generate whole datasets. Deep learning classification models were then trained on these datasets, utilizing transfer learning with the pre-trained AlexNet model.
...
The Performance Score was calculated based on three components: the model training accuracy, the model test accuracy on real data, and the MS-SSIM diversity of the dataset. Initially, any constants for each component were defined to facilitate model comparison. However, a hyperparameter could be incorporated to appropriately weight these components when calculating the score.\\
...
Explore a methodology to formulate conditions that verify whether a data augmentation technique increases dataset diversity to safe levels without compromising class separation.\\
...
\\
\\
\textit{Data availability.} The data are registered by \href{https://doi.org/10.5281/Zenodo.6001869}{\color{blue}https://doi.org/10.5281/Zenodo.6001869} (Salazar et al., 2022). These can also be found in the link: \href{https://zenodo.org/record/6001870#.YkGXKffQ-Xl}{\color{blue}https://zenodo.org/record/6001870\#.YkGXKffQ-Xl} (last access: 24 February 2023).
\\


\section*{Acknowledgments}
We express our acknowledgements to the CKELAR-VOLCANES team.
