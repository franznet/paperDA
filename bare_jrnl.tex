...
\item \textit{Augmented:} Since all data is augmented, the comparison is made between the two parts resulting from dividing the dataset.
\end{itemize}
The metric is called an \textit{external} diversity and similarity metric if the real dataset and the balanced dataset are considered for the comparison (Table \ref{table:external_similarity_diversity_metrics}).

\subsection{Performace Score}
By generating a simple score that accounts for model training accuracy (higher values are better), model test accuracy on real data (higher values are better), and the MS-SSIM metric value (lower values are more favorable, indicating greater diversity), we define the Performance Score ($PS$) as follows:
\begin{equation}
...
To improve model performance, it is helpful to understand how model accuracy relates to the variation in diversity generated by the data augmentation method.\\
Explore a method of formulating conditions that verify whether the data augmentation technique increases diversity to safe levels without mixing classes.\\
...
\\
\\
\textit{Data availability.} The data are registered by \href{https://doi.org/10.5281/Zenodo.6001869}{\color{blue}https://doi.org/10.5281/Zenodo.6001869} (Salazar et al., 2022). These can be also founded in the link: \href{https://zenodo.org/record/6001870#.YkGXKffQ-Xl}{\color{blue}https://zenodo.org/record/6001870\#.YkGXKffQ-Xl} (last access: 24 February 2023).
\\
